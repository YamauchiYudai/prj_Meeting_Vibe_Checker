...
async def analyze_session_frame(session_id: UUID, request: AnalyzeRequest, db: AsyncSession = Depends(get_db)):
    stmt = select(Session).where(Session.id == session_id)
    result = await db.execute(stmt)
    db_session = result.scalars().first()
    if not db_session:
    ...
    if db_session.status != 'active':
        raise HTTPException(status_code=400, detail="Cannot analyze frames for ended session")
...
async def get_session_results(session_id: UUID, db: AsyncSession = Depends(get_db)):
    stmt = select(VibeRecord).where(VibeRecord.session_id == session_id).order_by(VibeRecord.recorded_at.asc())
    result = await db.execute(stmt)
